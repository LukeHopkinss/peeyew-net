...
type DomeGalleryProps = {
  ...
  /** NEW: if provided, the gallery will call this on tile click instead of running the built-in zoom overlay */
  onOpen?: (payload: { index: number; src: string; alt: string }) => void;
};
        ...
        if (last) {
          ...
          if (isTap && tapTargetRef.current && !focusedElRef.current) {
            ...
            if (onOpen && parent) {
              ...
              const alt = parent.dataset.alt || (tapTargetRef.current.querySelector('img') as HTMLImageElement)?.alt || '';
              const all = Array.from(parent?.parentElement?.parentElement?.querySelectorAll?.('.sphere-item') || []);
              const index = all.findIndex((el) => el === parent);
              onOpen({ index, src, alt });
            } else {
                ...
                >
                  <div
                    ...
                    onClick={(e) => {
                      ...
                      if (onOpen) {
                        onOpen({ index: i, src: it.src, alt: it.alt });
                      } else {
